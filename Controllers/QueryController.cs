...
namespace PoliziaMunicipale.Controllers
{
    public class QueryController : Controller
    {
        ...
        public IActionResult TrasgrePunti()
        {
            ...
            {
                DB.conn.Open();
                var cmd = new SqlCommand("SELECT v.IDAnagrafica, a.Cognome, a.Nome,  sum(v.DecurtamentoPunti) as TotPunti\r\n  FROM [dbo].[Verbale] as v\r\n  join dbo.Anagrafica as a on a.IDAnagrafica = v.IDAnagrafica \r\n  Group by v.IDAnagrafica, a.Cognome, a.Nome", DB.conn);
                var reader = cmd.ExecuteReader();
                if (reader.HasRows)
                {
                    while (reader.Read())
